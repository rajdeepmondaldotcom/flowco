          ...
          {fx && (
            <>
              ...
              <div className="mb-5 rounded-md border border-line">
                ...
                {fx.impliedRate !== null && (
                  <ReconRow
                    label="Implied rate (claim ÷ receipt)"
                ...
                <div
                  className={`border-t border-line px-3 py-2 text-xs font-semibold uppercase tracking-wide ${
                    fx.plausible ? "bg-clear-soft text-clear" : "bg-danger-soft text-danger"
                  }`}
                >
                  {fx.plausible ? "rate checks out against the reference" : "implied rate looks off"}
                </div>
...
function ReconRow({ label, value, alert, strong }: { label: string; value: string; alert?: boolean; strong?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-4 border-b border-line px-3 py-1.5 text-sm last:border-b-0">
      <span className="shrink-0 text-ink-soft">{label}</span>
      <span
        className={`figure min-w-0 text-right ${alert ? "font-semibold text-danger" : strong ? "font-semibold text-ink" : ""}`}
      >
        {value}
      </span>
    </div>

...
export default function TriageApp() {
  ...
  const runTriage = useCallback(async () => {
    ...
    // Concurrent workers each hit failures independently; collect them all so
    // one error can't silently overwrite another.
    ...
    if (failures.length === 1) {
      ...
      setError(`${failures.length} cases didn't finish (last error: ${failures[failures.length - 1]}). They're back in Awaiting triage — run again to retry.`);
    }
    await refresh();
    setRunning(false);
  ...
  const approveAllClear = useCallback(async () => {
    ...
    // Replace the per-item toasts with one clear summary + undo-all, and never
    // let failures scroll silently out of the toast queue.
    if (ids.length > 1) {
      toast({
        title: `Approved ${ids.length} expenses`,
        description: failed > 0 ? `${failed} didn't go through — they're still in the lane` : "The whole clear lane",
        tone: "success",
        action: { label: "Undo all", onClick: () => ids.forEach((id) => revert(id)) },
      });
    } else if (failed > 0) {
      toast({
        title: `${failed} approval${failed === 1 ? "" : "s"} didn't go through`,
        description: "Still in the lane — try again",
        tone: "danger",
      });
    }
  }, [lanes.clear, act, toast, revert]);

  // Must match the on-screen lane order (clear, review, untriaged, resolved)
  // so j/k walk the page top to bottom instead of zigzagging.
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (showShortcuts) {
        // The overlay owns the keyboard while open: Esc closes, everything
        // else is ignored so the queue cursor can't move behind the modal.
          ...
          <>
            {lanes.clear.length > 0 && (
              <Lane
                ...
                subtitle="Every check passed. The assistant found nothing to question. Clear the lane in one click."
                tone="clear"
            ...
            {lanes.review.length > 0 && (
              <Lane
                ...
                subtitle="The assistant did the digging but could not settle these. The proof is inside."
                tone="flag"
            ...
            {lanes.untriaged.length > 0 && (
              <Lane
                ...
                subtitle="Sent in, but the assistant has not read them yet."
                tone="neutral"
  ...
  return (
    <div className="fade-in mb-5 overflow-hidden rounded-xl border border-accent/25 bg-accent-soft/60">
      ...
      <div className="grid grid-cols-1 border-t border-accent/15 text-xs text-ink-soft sm:grid-cols-3">
        <Step n="1" title="The assistant digs" body="It reads the receipt, checks the math, and runs every policy check." />
        <Step n="2" title="Clean ones clear" body="Under the cap, receipt matches, nothing odd: the one-click lane." divide />
        <Step n="3" title="You decide the rest" body="Unclear, over cap, or a possible double: the proof is laid out for you." divide />
      </div>
    </div>
  );
}
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <div className="mt-0.5 min-w-0 truncate text-xs text-ink-faint">
          ...
          {busy ? "assistant is reading…" : expense.aiVerdict?.summary ?? expense.purpose}
        </div>
        {flags.length > 0 && (
